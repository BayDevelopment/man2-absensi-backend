...
class AbsensiController extends Controller
{
    ...
    private const FACE_THRESHOLD = 0.42;
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->whereHas('jadwal.mataPelajaran', fn($mp) => $mp->where('nama', 'like', "%{$search}%"))
                    ->orWhereHas('jadwal.guru', fn($g) => $g->where('nama_lengkap', 'like', "%{$search}%")
        ...
        $data = $data
            ->sortBy(fn($row) => ($row['tanggal'] ?? '0000-00-00') . ' ' . ($row['jam_jadwal_mulai'] ?? '00:00:00'))
            ->values();
    ...
    public function absenMasuk(Request $request)
    {
        ...
        $jadwal = JadwalModel::with(['mataPelajaran', 'guru'])
            ...
            ->where(fn($q) => $this->applyHariFilter($q, $now))
        ...
        if (!$isFirstMapel) {
            return response()->json([
                ...
                'message' => 'Siswa hanya dapat melakukan absen pada jam masuk pagi.',
            ], 422);
        }
        ...
        if ($jamSekarang > $batas['jam_tutup']) {
            return response()->json([
                'success' => false,
                'type'    => 'attendance_closed',
                'message' => 'Mapel sudah selesai. Tidak bisa lagi melakukan absen masuk.',
            ], 422);
        }
        ...
        $statusKehadiran = $jamSekarang > $batas['batas_terlambat'] ? 'terlambat' : 'hadir';

        // ─── KETERANGAN OTOMATIS ───────────────────────────────────────────
        // hadir  → null (kosong, tidak perlu keterangan)
        // terlambat → pesan otomatis dengan info jam masuk & batas terlambat
        $jamSekolahAktif = $this->activeJamSekolah();
        $batasTerlambatLabel = $jamSekolahAktif?->batas_terlambat
            ? substr($this->timeOnly($jamSekolahAktif->batas_terlambat), 0, 5)
            : substr($batas['batas_terlambat'], 0, 5); // fallback jika jam_sekolah tidak ada

        $keteranganOtomatis = match ($statusKehadiran) {
            'terlambat' => 'Absen terlambat (batas ' . $batasTerlambatLabel . ', masuk ' . substr($jamSekarang, 0, 5) . ')',
            default     => null,
        };
        // ──────────────────────────────────────────────────────────────────
        ...
        try {
            $absensi = DB::transaction(function () use (
                ...
                return AbsensiModel::create([
                    ...
                    // ✅ keterangan otomatis: isi jika terlambat, null jika hadir
    ...
    public function absenKeluar(Request $request)
    {
        return response()->json([
            ...
            'message' => 'Siswa hanya melakukan absen masuk pagi. Jam keluar dikelola guru.',
        ], 422);
    }
    ...
    public function simpanStatus(Request $request)
    {
        ...
        if (!$siswa) {
            return response()->json(['success' => false, 'type' => 'siswa_not_found', 'message' => 'Data siswa tidak ditemukan.'], 404);
        }
        ...
        if (!$kelasId) {
            return response()->json(['success' => false, 'type' => 'kelas_not_found', 'message' => 'Data kelas siswa tidak ditemukan.'], 422);
        }
        ...
        if ($tanggal !== $now->toDateString()) {
            return response()->json(['success' => false, 'type' => 'invalid_date', 'message' => 'Izin/Sakit hanya bisa diajukan untuk hari ini.'], 422);
        }
        ...
        if (!$jadwal) {
            return response()->json(['success' => false, 'type' => 'invalid_schedule', 'message' => 'Jadwal tidak valid.'], 422);
        }

        if (!$this->isFirstMapelToday($siswa, $jadwal, $now)) {
            return response()->json(['success' => false, 'type' => 'schedule_not_first_mapel', 'message' => 'Izin/Sakit hanya dapat diajukan pada jadwal absen pagi.'], 422);
        }

        $batas = $this->getBatasAbsensi($jadwal, $siswa, $now);

        if ($jamSekarang < $batas['jam_buka']) {
            return response()->json(['success' => false, 'type' => 'cannot_request_status', 'message' => 'Pengajuan belum dibuka. Silakan ajukan mulai pukul ' . substr($batas['jam_buka'], 0, 5) . '.'], 422);
        }

        if ($jamSekarang > $batas['jam_tutup']) {
            return response()->json(['success' => false, 'type' => 'attendance_closed', 'message' => 'Batas pengajuan izin/sakit sudah lewat. Silakan hubungi guru.'], 422);
        }
        ...
        if ($existing && in_array($existing->status, ['hadir', 'terlambat'], true)) {
            return response()->json(['success' => false, 'type' => 'already_present', 'message' => 'Anda sudah diabsen hadir di mapel ini.'], 422);
        }

        if ($existing && $existing->jam_masuk) {
            return response()->json(['success' => false, 'type' => 'already_present', 'message' => 'Absensi sudah memiliki jam masuk, status tidak bisa diubah.'], 422);
        }
        ...
        // Jika user mengisi keterangan → pakai keterangan user
        // Jika kosong → buat keterangan otomatis sesuai status
        $keteranganUser = trim($request->input('keterangan', ''));
        $keteranganFinal = $keteranganUser !== ''
            ? $keteranganUser
        ...
        try {
            $absensi = DB::transaction(function () use (
                ...
                return AbsensiModel::updateOrCreate(
                    ...
                    [
                        ...
                        // ✅ keterangan dari user, atau otomatis jika kosong
            ...
            return response()->json(['success' => false, 'type' => 'server_error', 'message' => 'Gagal menyimpan: ' . $e->getMessage()], 500);
        }
    }
    ...
    private function appendTodayScheduleRows($data, SiswaModel $siswa, Carbon $now)
    {
        ...
        foreach ($jadwalHariIni as $index => $jadwal) {
            $exists = $data->first(
                fn($item) => ($item['tanggal'] ?? null) === $tanggalHariIni &&
    ...
    private function formatAbsensiRow(AbsensiModel $absensi, ?Carbon $now = null): array
    {
        ...
        $mapel = $jadwal?->mataPelajaran?->nama ?? $jadwal?->matpel?->nama ?? $jadwal?->mapel?->nama ?? $jadwal?->nama_mapel ?? '—';
        $guru  = $jadwal?->guru?->nama_lengkap ?? $jadwal?->guru?->nama ?? $jadwal?->guru?->name ?? '—';

        $kelasNama = $absensi->kelas?->nama_kelas ?? $absensi->kelas?->nama
            ?? $absensi->siswa?->kelas?->nama_kelas ?? $absensi->siswa?->kelas?->nama ?? '—';

        $batas = $jadwal
            ? $this->getBatasAbsensi($jadwal, $absensi->siswa ?? $this->fakeSiswaFromAbsensi($absensi), $now)
            : ['jam_buka' => null, 'batas_terlambat' => null, 'jam_tutup' => null, 'is_first_mapel' => false];
        ...
        // Prioritas:
        // 1. Keterangan dari DB (diisi saat absen/izin/sakit)
        // 2. Kosong string jika null (bukan "—", agar kolom tabel bersih)
        $keteranganTampil = $absensi->keterangan ?? '';
        // ──────────────────────────────────────────────────────────────────
        ...
        return [
            ...
            'guru'        => $guru,
            'kelas'       => ['id' => $absensi->kelas_id, 'nama_kelas' => $kelasNama, 'nama' => $kelasNama],
            ...
            'status'     => $status,
            // ✅ keterangan dari DB saja — tidak di-override action text
            'keterangan' => $keteranganTampil,
            ...
            'is_mapel_selesai' => $tanggal === $now->toDateString() && $jamSelesai && $jamSekarang > $jamSelesai,

            'action_state' => 'sudah_tercatat',
            // ✅ action_text tetap ada untuk UI tombol, tapi TIDAK masuk ke kolom keterangan
            ...
            'jadwal' => [
                ...
                'guru'        => ['nama_lengkap' => $guru, 'nama' => $guru, 'name' => $guru],
            ],
        ];
    }

    private function formatScheduleRowWithoutAbsensi(SiswaModel $siswa, JadwalModel $jadwal, Carbon $now, bool $isFirstMapel): array
    {
        ...
        $jamTutup       = $batas['jam_tutup'] ?? $this->timeOnly($jadwal->jam_selesai);
        ...
        $canAbsenPagi = $isFirstMapel && !$isBelumDibuka && !$isMapelSelesai;
        ...
        return [
            ...
            'mata_pelajaran' => $mapel,
            'guru'        => $guru,
            'kelas'       => ['id' => $this->siswaKelasId($siswa), 'nama_kelas' => $kelasNama, 'nama' => $kelasNama],
            'nama_kelas'  => $kelasNama,
            ...
            'status'     => 'alfa',
            // ✅ baris tanpa absensi: keterangan kosong, actionText untuk UI saja
            'keterangan' => '',
            ...
            'is_attendance_closed' => (bool) $isMapelSelesai,
    ...
    private function currentSiswa(Request $request): ?SiswaModel
    {
        ...
        // Eksplisit select kolom yang dibutuhkan termasuk face_descriptor
        $query = SiswaModel::with('kelas')
    ...
    private function getBatasAbsensi(JadwalModel $jadwal, SiswaModel $siswa, Carbon $date): array
    {
        ...
        $jamTutup = $this->timeOnly($jadwal->jam_selesai) ?? '23:59:59';
    ...
    {
        return JamSekolahModel::query()->where('aktif', '=', 1)->first();
    }
    ...
    private function buildStats($rows): array
    {
        ...
        return [
            ...
            'total_alfa'           => $alfa,
            'persentase_kehadiran' => $total > 0 ? round((($hadir + $terlambat) / $total) * 100) : 0,
        ];
    }
    ...
    private function timeOnly($value): ?string
    {
        ...
        try {
            ...
            $value = (string) $value;
            if (strlen($value) === 5) return $value . ':00';
    ...
    private function normalizeDescriptor($descriptor): ?array
    {
        ...
        if (!is_array($descriptor) || count($descriptor) !== 128) return null;

        $normalized = [];
        foreach (array_values($descriptor) as $value) {
            if (!is_numeric($value)) return null;
            $float = (float) $value;
            if (!is_finite($float) || abs($float) > 10) return null;
            $normalized[] = $float;
        }
    ...
    private function compareFaceDescriptors(?array $registered, ?array $incoming): array
    {
        if (!$registered || !$incoming || count($registered) !== 128 || count($incoming) !== 128) {
            return ['matched' => false, 'distance' => null, 'confidence' => null];
        }

        $sum = 0.0;
        for ($i = 0; $i < 128; $i++) {
            $diff = (float) $registered[$i] - (float) $incoming[$i];
            $sum += $diff * $diff;
        }
